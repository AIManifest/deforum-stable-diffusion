...
def load_controlnet(controlnet_config_path, controlnet_model_path):
    ...
    # model.load_state_dict(load_state_dict('/content/control_sd15_hed.pth', location='cuda'))
...
def process(control, input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, save_memory=False):
    # controlnet_config_path = control.controlnet_config_path
    # controlnet_model_path = control.controlnet_model_path
    # print_flag = False
    # verbose = False
    # model = create_model(controlnet_config_path).cpu()
    # # model.load_state_dict(load_state_dict('/content/control_sd15_hed.pth', location='cuda'))
...
def generate_control(control):
    ...
    while img_idx < control.max_frames:
        ...
        image = process(control, input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta)
